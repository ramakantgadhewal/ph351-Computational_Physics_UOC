...
int main()
{
    ...
    const int reps = 500;
    constexpr double xmin{-4}, xmax{4};
    constexpr double ymin{-4}, ymax{4};
    constexpr int N = 50;
    constexpr int M = 50;
    ...
    for(int i=0; i<N; i++)
    {
        for(int j=0; j<M; j++)
        {
            S[S.Index(i,j)] = std::sin( 2 * M_PI * x[j] / 8 ) * std::exp( - std::pow(y[i], 2) / 10 );
        }
    }
    ...
    for(int i=0; i<phi.NumColumns(); i++)
    {
        for(int j=0; j<phi.NumRows(); j++)
        {
            ...
            if(j==0) phi[phi.Index(i,j)] = -1;
            else if(j==phi.NumRows()-1) phi[phi.Index(i,j)] = 1;
        }
    }
    ...
    fs::create_directory(p+="/m1_data_2d");

    std::ofstream energy{"m1_data_2d/energy_data_2d.dat"};
    ...
    std::ofstream pot{"m1_data_2d/potential_2d.dat"};
    for(int i=0; i<phi.NumColumns(); i++)
    {
        pot << std::endl;
        for(int j=0; j<phi.NumRows(); j++)
    ...
    std::ofstream Fieldx{"m1_data_2d/fieldx_2d.dat"};
    ...
    std::ofstream Fieldy{"m1_data_2d/fieldy_2d.dat"};
    ...
    std::ofstream Source{"m1_data_2d/source_2d.dat"};
